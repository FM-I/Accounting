...
{
    public partial class OutBankAccontOrderElementForm : Window, INotifyPropertyChanged
    {
        ...
        private OutBankAccontOrder _data = new();
        ...
        public OutBankAccontOrderElementForm(Guid id = default)
        {
            ...
            if (id != default)
            {
                var data = _documentController.GetDocument<OutBankAccontOrder>(id);
                if (data != null)
                {
                    _data = data;
        ...
        public OutBankAccontOrderElementForm(OutBankAccontOrder data)
        {
            _handbookController = DIContainer.ServiceProvider.GetRequiredService<IHandbookController>();
            _documentController = DIContainer.ServiceProvider.GetRequiredService<IDocumentController>();
            _infoController = DIContainer.ServiceProvider.GetRequiredService<IInformationRegisterController>();
        ...
        private void btnClearBankAccount_Click(object sender, RoutedEventArgs e)
        {
            _data.Account = null;
            BankAccountName = "";
        }

        private void btnShowListBankAccount_Click(object sender, RoutedEventArgs e)
        {
            var form = new BankAccountListForm(true);
            if (form.ShowDialog() != null)
            {
                if (form.SelectedId != default)
                {
                    var data = _handbookController.GetHandbook<BankAccount>(form.SelectedId);
                    if (data != null)
                    {
                        _data.Account = data;
        ...
        private void btnOpenBankAccount_Click(object sender, RoutedEventArgs e)
        {
            if (_data.Account == null)
                return;

            var form = new BankAccountElementForm(_data.Account.Id);
            if (form.ShowDialog() != null)
            {
                var data = _handbookController.GetHandbook<BankAccount>(_data.Account.Id);
                if (data != null)
                {
                    _data.Account = data;
                    BankAccountName = data.Name;
